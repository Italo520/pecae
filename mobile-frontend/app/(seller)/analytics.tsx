...
export default function AnalyticsScreen() {
  ...
  const chartData = data?.historico?.map(item => {
    ...
    const dateObj = new Date(item.dataReferencia);
    const day = String(dateObj.getUTCDate()).padStart(2, '0');
    const month = String(dateObj.getUTCMonth() + 1).padStart(2, '0');
    return {
      value: item.totalVisualizacoes,
      label: `${day}/${month}`,
    };
  }) || [];

  const conversionRate = data?.taxaConversao != null ? data.taxaConversao.toFixed(1) : '0.0';
              ...
              <View style={styles.metricsGrid}>
                <View style={styles.metricsRow}>
                  <MetricCard 
                    icon="eye" 
                    label="Visualizações" 
                    value={data?.totalVisualizacoes?.toLocaleString('pt-BR') || '0'} 
                  />
                  <MetricCard 
                    icon="chatbubble-ellipses" 
                    label="Contatos Iniciados" 
                    value={data?.totalContatos?.toLocaleString('pt-BR') || '0'} 
                  />
                </View>
                <View style={styles.metricsRow}>
                  <MetricCard 
