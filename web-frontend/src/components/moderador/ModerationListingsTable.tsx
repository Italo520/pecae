    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="relative max-w-md w-full">
          ...
          <input
            type="text"
            className="block w-full pl-10 pr-3 py-2 border border-[var(--border)] rounded-xl leading-5 bg-[var(--surface)] text-[var(--foreground)] placeholder-[var(--muted)] focus:outline-none focus:border-[var(--brand)]/50 sm:text-sm transition-colors"
        ...
        {selectedIds.length > 0 && (
          <div className="flex items-center gap-3 bg-[var(--brand)]/10 px-4 py-2 rounded-xl border border-[var(--brand)]/20 animate-in fade-in slide-in-from-bottom-2">
            <span className="text-sm font-medium text-[var(--brand)]">
              {selectedIds.length} selecionados
            </span>
            <div className="w-px h-4 bg-[var(--brand)]/20 mx-1" />
            <button
            ...
            <button
              ...
              disabled={isApproving}
              className="text-xs font-semibold text-[var(--brand)] hover:opacity-80 transition-colors flex items-center gap-1"
            >
      ...
      {/* Table */}
      <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl overflow-hidden backdrop-blur-xl">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-[var(--border)] bg-[var(--background)]/50 text-sm font-medium text-[var(--muted)]">
                <th className="p-4 w-12">
                  <input 
                    type="checkbox" 
                    className="rounded border-[var(--border)] bg-[var(--background)] text-[var(--brand)] focus:ring-[var(--brand)]/50"
              ...
              {filteredListings.length === 0 ? (
                ...
                filteredListings.map((listing) => {
                  ...
                  return (
                    <tr 
                      key={listing.id} 
                      className={`transition-colors hover:bg-[var(--surface-hover)] ${isSelected ? 'bg-[var(--brand)]/10' : ''}`}
                    >
                      <td className="p-4">
                        <input 
                          type="checkbox" 
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          <div className="w-12 h-12 rounded-lg bg-[var(--background)] border border-[var(--border)] overflow-hidden flex-shrink-0 flex items-center justify-center relative">
                            {imageUrl ? (
                          ...
                          <div>
                            <p className="font-medium text-[var(--foreground)] text-sm line-clamp-1">{listing.titulo}</p>
                            <p className="text-xs text-[var(--muted)] mt-0.5">{listing.veiculo?.brand} {listing.veiculo?.model} • {listing.veiculo?.year}</p>
                          </div>
                        </div>
                      </td>
                      <td className="p-4 hidden md:table-cell">
                        <div className="text-sm text-[var(--foreground)]">{listing.seller?.nomeFantasia}</div>
                      ...
                      <td className="p-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            ...
                            className="p-2 rounded-lg text-[var(--muted)] hover:text-[var(--brand)] hover:bg-[var(--brand)]/10 transition-colors"
                            title="Aprovar"
                          >
