...
export class AnalyticsService {
  ...
  constructor(
    ...
    this.hashSalt = this.configService.get<string>('ANALYTICS_HASH_SALT', 'pecae_local_hash_fallback_salt');
  }
  ...
  async getSellerAnalytics(userId: string, periodDays: number = 30) {
    ...
    const viewsTimeline = Object.entries(viewsTimelineMap).map(([date, count]) => ({
      date,
      count,
  ...
  async getAdminAnalytics() {
    ...
    const listingStatusAggregation = await this.prisma.listing.groupBy({
      by: ['status'],
      _count: {
        _all: true,
      },
    ...
    const listingsByStatus = listingStatusAggregation.reduce((acc, curr) => {
    ...
    const pendingVerifications = await this.prisma.sellerVerification.count({
      where: { status: 'PENDING' },
    });
